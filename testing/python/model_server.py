...
def init_model(model_type):
    global net 
    net = caffe.Net(model['deployFile_'+model_type], model['caffemodel_'+model_type], caffe.TEST)

def predict_image(oriImg, model_type):
...
#    to_plot = cv.addWeighted(testImg, 0.3, oriImg, 0.7, 0)
    ...
    print 'This pic took %.2f ms.\n' % (1000 * (time.time() - start_time_all))
    return canvas
    
 
if __name__ == "__main__":
    oriImg = cv.imread('input/one_people.jpg')
    oriImg = cv.resize(oriImg, (fixed_size,fixed_size), interpolation=cv.INTER_LINEAR) 
    pic = predict_image(oriImg, 's')
    print pic.shape
